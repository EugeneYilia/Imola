...
def ask_with_context_stream(user_question, collection_name, model="mistral:7b-instruct", top_k=SystemConfig.default_top_k):
    ...
    1. 回答中请完全使用中文，**不要出现任何英文单词或术语**（例如 depth、hole、spacing 等），**也不要加括号补充英文原文**；
    2. 回答内容应适当断句，**每句话不超过10个字**；
        - 所有内容，包括**数字、单位、标点、括号、引号**，**都应计入字数统计**；
        - 一句话如果包含括号、引号、或数字，也算入整句话长度；
    3. 遇到较长内容，应**主动拆分为多句短句**，句子之间使用中文标点隔开；
    4. 不要输出术语定义、英文注释或任何附加解释；
    5. 回答风格应**简洁、专业、自然、口语化**，便于朗读和语音合成。
